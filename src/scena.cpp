...
for (int i = 0; i < 4; i++){
  ...
  int nr_p=rand()%3;
  if(nr_p==0)
  lista_przeszkod.push_front(std::make_shared<goraSzczyt>(losowy_srodek, 100, 50, 100, "../datas/Element" + std::to_string(numer_elementu) + ".dat"));
  else if(nr_p==1)
  lista_przeszkod.push_front(std::make_shared<plaskowyz>(losowy_srodek, 100, 50, 100, "../datas/Element" + std::to_string(numer_elementu) + ".dat"));
  else if(nr_p==2)
  lista_przeszkod.push_front(std::make_shared<goraGran>(losowy_srodek, 100, 50, 100, "../datas/Element" + std::to_string(numer_elementu) + ".dat"));
  numer_elementu++;
  
  
  for (std::list<std::shared_ptr<Solid>>::const_iterator a = lista_przeszkod.begin(); a != lista_przeszkod.end(); a++)
  {
    Lacze.DodajNazwePliku((*a)->jaka_nazwa().c_str());
    (*a)->zapisz_do_pliku();
  }
